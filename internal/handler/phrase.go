...
var dummyPhrasePlaylists = []dummyPlaylist{
	{
		Name:        "Frases Básicas en Inglés",
		Description: "Saludos, preguntas comunes y despedidas para el día a día.",
		Language:    "en",
		Groups: []dummyGroup{
			{
				Name: "Saludos",
				Phrases: []dummyPhrase{
					{Text: "Hello!", TranslationES: "¡Hola!", PronunciationES: "jelou"},
					{Text: "Good morning", TranslationES: "Buenos días", PronunciationES: "gud mornin"},
					{Text: "Good afternoon", TranslationES: "Buenas tardes", PronunciationES: "gud afternun"},
					{Text: "Nice to meet you", TranslationES: "Encantado de conocerte", PronunciationES: "nais tu mit yu"},
				},
			},
			{
				Name: "Preguntas Comunes",
				Phrases: []dummyPhrase{
					{Text: "How are you?", TranslationES: "¿Cómo estás?", PronunciationES: "jau ar yu"},
					{Text: "What is your name?", TranslationES: "¿Cuál es tu nombre?", PronunciationES: "juat is yur neim"},
					{Text: "Where are you from?", TranslationES: "¿De dónde eres?", PronunciationES: "juer ar yu from"},
				},
			},
			{
				Name: "Despedidas",
				Phrases: []dummyPhrase{
					{Text: "Goodbye", TranslationES: "Adiós", PronunciationES: "gudbai"},
					{Text: "See you later", TranslationES: "Hasta luego", PronunciationES: "si yu leiter"},
					{Text: "Have a nice day", TranslationES: "Que tengas un buen día", PronunciationES: "jav a nais dei"},
				},
			},
		},
	},
	{
		Name:        "Frases de Viaje en Inglés",
		Description: "Frases útiles para aeropuerto, hotel y restaurante.",
		Language:    "en",
		Groups: []dummyGroup{
			{
				Name: "Aeropuerto",
				Phrases: []dummyPhrase{
					{Text: "Where is the boarding gate?", TranslationES: "¿Dónde está la puerta de embarque?", PronunciationES: "juer is de bordin geit"},
					{Text: "I lost my luggage", TranslationES: "Perdí mi equipaje", PronunciationES: "ai lost mai laguich"},
					{Text: "What time does the flight leave?", TranslationES: "¿A qué hora sale el vuelo?", PronunciationES: "juat taim das de flait liv"},
				},
			},
			{
				Name: "Hotel",
				Phrases: []dummyPhrase{
					{Text: "I have a reservation", TranslationES: "Tengo una reservación", PronunciationES: "ai jav a reserveishon"},
					{Text: "Is breakfast included?", TranslationES: "¿El desayuno está incluido?", PronunciationES: "is brekfast included"},
					{Text: "Can I have the key, please?", TranslationES: "¿Me da la llave, por favor?", PronunciationES: "can ai jav de ki, plis"},
				},
			},
			{
				Name: "Restaurante",
				Phrases: []dummyPhrase{
					{Text: "Can I see the menu?", TranslationES: "¿Puedo ver el menú?", PronunciationES: "can ai si de meniu"},
					{Text: "The check, please", TranslationES: "La cuenta, por favor", PronunciationES: "de chek, plis"},
					{Text: "I am vegetarian", TranslationES: "Soy vegetariano", PronunciationES: "ai am vegueterian"},
				},
			},
		},
	},
	{
		Name:        "Frases Básicas en Portugués",
		Description: "Saludos y preguntas frecuentes en portugués.",
		Language:    "pt",
		Groups: []dummyGroup{
			{
				Name: "Saludos",
				Phrases: []dummyPhrase{
					{Text: "Olá!", TranslationES: "¡Hola!", PronunciationES: "olá"},
					{Text: "Bom dia", TranslationES: "Buenos días", PronunciationES: "bon dia"},
					{Text: "Boa tarde", TranslationES: "Buenas tardes", PronunciationES: "boa tarde"},
				},
			},
			{
				Name: "Preguntas Frecuentes",
				Phrases: []dummyPhrase{
					{Text: "Como você está?", TranslationES: "¿Cómo estás?", PronunciationES: "como vosé está"},
					{Text: "Qual é o seu nome?", TranslationES: "¿Cuál es tu nombre?", PronunciationES: "qual é o seu nome"},
					{Text: "Onde fica o banheiro?", TranslationES: "¿Dónde está el baño?", PronunciationES: "onchi fica o baniero"},
				},
			},
		},
	},
...
// POST /api/phrase-playlists/seed-dummy
// Inserts 3 dummy phrase playlists with groups and phrases for the current user.
func (h *PhraseHandler) SeedDummy(w http.ResponseWriter, r *http.Request) {
	userID, err := h.userIDFromContext(r)
	if err != nil {
		jsonError(w, "unauthorized", http.StatusUnauthorized)
		return
	...
	tx, err := h.db.Begin()
	if err != nil {
		log.Printf("SeedDummy begin tx: %v", err)
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	}
	defer tx.Rollback()
	...
	for _, pl := range dummyPhrasePlaylists {
		var playlistID int
		err := tx.QueryRow(
			`INSERT INTO phrase_playlists (user_id, name, description, language)
			 VALUES ($1, $2, $3, $4) RETURNING id`,
			userID, pl.Name, pl.Description, pl.Language,
		).Scan(&playlistID)
		if err != nil {
			log.Printf("SeedDummy insert playlist: %v", err)
			jsonError(w, fmt.Sprintf("failed to insert playlist %q", pl.Name), http.StatusInternalServerError)
			return
		}
		...
		for gi, g := range pl.Groups {
			...
			err := tx.QueryRow(
				...
				playlistID, g.Name, gi,
			).Scan(&groupID)
			if err != nil {
				log.Printf("SeedDummy insert group: %v", err)
				jsonError(w, "failed to insert group", http.StatusInternalServerError)
				return
			}

			for pi, ph := range g.Phrases {
				_, err := tx.Exec(
					`INSERT INTO phrases (phrase_group_id, text, translation_es, pronunciation_es, position)
					 VALUES ($1, $2, $3, $4, $5)`,
					groupID, ph.Text, ph.TranslationES, ph.PronunciationES, pi,
				)
				if err != nil {
					log.Printf("SeedDummy insert phrase: %v", err)
					jsonError(w, "failed to insert phrase", http.StatusInternalServerError)
					return
				}
				phraseCount++
		...
		created = append(created, createdPlaylist{
			ID: playlistID, Name: pl.Name, Groups: len(pl.Groups), Phrases: phraseCount,
		})
	}

	if err := tx.Commit(); err != nil {
		log.Printf("SeedDummy commit: %v", err)
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	}
